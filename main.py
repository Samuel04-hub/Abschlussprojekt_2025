"""
import streamlit as st
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
import pydeck as pdk
import os
from PIL import Image
from datetime import date
from src.analyze_fit_files_tour import analyze_all_fit_files
from src.analyze_hoehenkurve import analyze_wildschoenau, analyze_pillersee
from src.analyze_data_GPS import analyze_wildschoenau as analyze_gps_wildschoenau, analyze_pillersee as analyze_gps_pillersee
from src.ekgdata import EKGdata
from src.person import Person
from src.analyze_activity_data import dataplot, dataframe
from src.analyze_anomalien import analyze_ecg_anomalies

# Streamlit-Seitenkonfiguration
st.set_page_config(page_title="Touren & EKG Analyse", layout="centered")

# Verzeichnis für Bild-Uploads
IMAGE_DIR = "data/images"
os.makedirs(IMAGE_DIR, exist_ok=True)

# Sidebar für Navigation
page = st.sidebar.selectbox("Wähle eine Analyse", ["Touren-Analyse", "Personen- & EKG-Verwaltung", "EKG-Anomalie-Analyse"])

# --- Touren-Analyse (FIT, GPX, Höhenkurve) ---
if page == "Touren-Analyse":
    st.header("Touren-Analyse")

    # Tourauswahl
    tour = st.selectbox("Wähle eine Tour", ("Wildschönau", "Pillersee"), key="tour_select")

    # --- FIT-Dateien Analyse ---
    directory = "C:/Abschlussprojekt_2025/fit-files"

    if not os.path.exists(directory):
        st.error(f"Das Verzeichnis {directory} existiert nicht.")
        st.stop()

    fit_files = [f for f in os.listdir(directory) if f.endswith('.csv')]
    if not fit_files:
        st.error(f"Keine CSV-Dateien im Verzeichnis {directory} gefunden.")
        st.stop()

    results = analyze_all_fit_files(directory)
    if not results:
        st.error("Keine gültigen Dateien konnten verarbeitet werden.")
        st.stop()

    # Flexibler Dateinamenabgleich
    tour_lower = tour.lower().replace('ö', 'oe')
    selected_result = None
    for r in results:
        file_name_lower = r['file_name'].lower()
        if tour_lower in file_name_lower or file_name_lower.startswith(tour_lower[:5]):
            selected_result = r
            break

    if not selected_result:
        st.error(f"Keine Daten für die ausgewählte Tour {tour} gefunden.")
        st.stop()

    # --- GPX-Analyse und Karte ---
    st.subheader("Karte")
    try:
        track_data = analyze_gps_wildschoenau() if tour == "Wildschönau" else analyze_gps_pillersee()
        if track_data.get("coordinates"):
            lats = [lat for lon, lat in track_data["coordinates"]]
            lons = [lon for lon, lat in track_data["coordinates"]]
            midpoint = [sum(lats)/len(lats), sum(lons)/len(lons)]
            layer = pdk.Layer(
                "PathLayer",
                data=[{"path": track_data["coordinates"], "name": tour}],
                get_path="path",
                get_color=[255, 0, 0],
                width_scale=10,
                width_min_pixels=3,
            )
            view_state = pdk.ViewState(
                latitude=midpoint[0],
                longitude=midpoint[1],
                zoom=13,
                pitch=0,
            )
            st.pydeck_chart(
                pdk.Deck(
                    layers=[layer],
                    initial_view_state=view_state,
                    map_style="mapbox://styles/mapbox/outdoors-v11"
                )
            )
        else:
            st.write("Keine Koordinaten gefunden.")
    except Exception as e:
        st.error(f"Fehler beim Laden der Kartendaten: {e}")
        st.stop()

    # --- Zusammenfassung und Diagramme ---
    st.subheader("Zusammenfassung")
    table_data = {
        "Tour": [tour],
        "Höhenmeter (m)": [selected_result.get('elevation_gain_m', 0)],
        "Gesamtdistanz (km)": [selected_result.get('total_distance_km', 0)],
        "Durchschnittsherzfrequenz (bpm)": [round(sum(selected_result.get('heart_rate', [])) / len(selected_result['heart_rate']), 2) if selected_result.get('heart_rate') else 0],
        "Durchschnittsleistung (Watt)": [round(sum(selected_result.get('power', [])) / len(selected_result['power']), 2) if selected_result.get('power') else 0]
    }
    df = pd.DataFrame(table_data)
    st.dataframe(
        df,
        use_container_width=True,
        column_config={
            "Tour": st.column_config.TextColumn("Tour", width="medium"),
            "Höhenmeter (m)": st.column_config.NumberColumn("Höhenmeter (m)", format="%.2f"),
            "Gesamtdistanz (km)": st.column_config.NumberColumn("Gesamtdistanz (km)", format="%.2f"),
            "Durchschnittsherzfrequenz (bpm)": st.column_config.NumberColumn("Ø Herzfrequenz (bpm)", format="%.2f"),
            "Durchschnittsleistung (Watt)": st.column_config.NumberColumn("Ø Leistung (Watt)", format="%.2f")
        }
    )

    try:
        st.subheader("Herzfrequenz und Leistung")
        df_csv = pd.read_csv(f"{directory}/{selected_result['file_name']}")
        fig_combined = go.Figure()
        fig_combined.add_trace(
            go.Scatter(
                x=df_csv['Distance'],
                y=selected_result.get('heart_rate', []),
                mode='lines',
                name='Herzfrequenz',
                line=dict(color='blue'),
                yaxis='y1'
            )
        )
        fig_combined.add_trace(
            go.Scatter(
                x=df_csv['Distance'],
                y=selected_result.get('power', []),
                mode='lines',
                name='Leistung',
                line=dict(color='green'),
                yaxis='y2'
            )
        )
        fig_combined.update_layout(
            title="Herzfrequenz und Leistungsverlauf",
            xaxis=dict(title="Distanz (km)"),
            yaxis=dict(
                title=dict(text="Herzfrequenz (bpm)", font=dict(color="blue")),
                tickfont=dict(color="blue")
            ),
            yaxis2=dict(
                title=dict(text="Leistung (Watt)", font=dict(color="green")),
                tickfont=dict(color="green"),
                overlaying='y',
                side='right'
            ),
            hovermode='x unified',
            showlegend=True
        )
        st.plotly_chart(fig_combined, use_container_width=True)
    except Exception as e:
        st.error(f"Fehler beim Laden der Daten: {e}")

    # --- Höhenkurve ---
    st.subheader("Höhenprofil")
    try:
        df = analyze_wildschoenau() if tour == "Wildschönau" else analyze_pillersee()
    except Exception as e:
        st.error(f"Fehler beim Laden der Höhenkurve-Daten: {e}")
        st.stop()

    df = df.drop_duplicates(subset='Distance').sort_values('Distance')
    new_distance = pd.Series(range(int(df['Distance'].min() * 1000), 
                                  int(df['Distance'].max() * 1000) + 1)) / 1000
    df = pd.DataFrame({'Distance': new_distance}).merge(
        df[['Distance', 'AltitudeCorrected']], 
        on='Distance', 
        how='left'
    ).interpolate(method='linear')

    df['Slope'] = (df['AltitudeCorrected'].diff() / (df['Distance'].diff() * 1000)) * 100
    df['Slope'] = df['Slope'].fillna(0)

    fig = go.Figure(
        data=[
            go.Scatter(
                x=df['Distance'],
                y=df['AltitudeCorrected'],
                mode='lines',
                line=dict(color='blue', width=2),
                hovertemplate='Distanz: %{x:.2f} km<br>Höhe: %{y:.2f} m<br>Steigung: %{customdata:.2f}%',
                customdata=df['Slope']
            )
        ],
        layout=dict(
            title="Höhenprofil",
            xaxis_title="Distanz (km)",
            yaxis_title="Höhe (m)",
            showlegend=False,
            hovermode='x unified'
        )
    )
    st.plotly_chart(fig, use_container_width=True)

# --- Personen- & EKG-Verwaltung ---
elif page == "Personen- & EKG-Verwaltung":
    st.title("Personen- und EKG-Verwaltung")

    # Registerkarten für verschiedene Funktionen
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Person anzeigen", "Neue Person hinzufügen", "Person bearbeiten", "Person löschen", "EKG-Test hinzufügen"])

    # Personen laden
    persons = Person.get_person_data()
    person_names = Person.get_person_list(persons)

    with tab1:
        st.header("Person anzeigen")
        if person_names:
            selected_name = st.selectbox("Wähle eine Person", options=person_names, key="view_person")
            selected_person = Person.find_person_data_by_name(selected_name)
            if selected_person:
                st.image(selected_person.get_image(), caption=selected_name)
                st.write(f"Geburtsjahr: {selected_person.date_of_birth}")
                st.write(f"Alter: {selected_person.calc_age()} Jahre")
                st.write(f"Geschlecht: {selected_person.gender}")
                st.write(f"Maximale Herzfrequenz: {selected_person.hr_max}")

                # EKG-Tests anzeigen
                if selected_person.ekg_tests:
                    selected_test = st.selectbox("Wähle einen EKG-Test", options=[str(i + 1) for i in range(len(selected_person.ekg_tests))], key="view_test")
                    testdatum = selected_person.ekg_tests[int(selected_test) - 1]["date"]
                    st.write(f"Datum des Tests: {testdatum}")

                    # Analyse-Plot anzeigen
                    hr_max = st.number_input("Maximale Herzfrequenz", min_value=100, max_value=250, value=int(selected_person.hr_max), step=1, key="view_hr_max")
                    st.plotly_chart(dataplot(hr_max))

                    # Zonenstatistik
                    st.header("Zonenstatistik")
                    zone_minutes = (dataframe["Zone"].value_counts() / 60)
                    zone_minutes.index.name = "Zone"
                    zone_minutes.name = "Dauer (Minuten)"
                    zone_power = dataframe.groupby("Zone")["PowerOriginal"].mean()
                    zone_stats = zone_minutes.to_frame().join(zone_power.rename("Ø Power (W)"))
                    st.dataframe(zone_stats)

                    # EKG-Analyse
                    ekg_obj = EKGdata(selected_person.ekg_tests[int(selected_test) - 1])
                    ekg_obj.find_peaks()
                    hr_est = ekg_obj.estimate_hr()
                    st.write(f"Geschätzte Herzfrequenz aus EKG: {hr_est:.1f} bpm")
                else:
                    st.write("Keine EKG-Daten für diese Person verfügbar.")
        else:
            st.write("Keine Personen verfügbar.")

    with tab2:
        st.header("Neue Person hinzufügen")
        with st.form("add_person_form"):
            new_id = st.number_input("ID", min_value=1, step=1)
            new_date_of_birth = st.number_input("Geburtsjahr", min_value=1900, max_value=date.today().year, step=1)
            new_firstname = st.text_input("Vorname")
            new_lastname = st.text_input("Nachname")
            new_gender = st.selectbox("Geschlecht", options=["Male", "Female", "Other"])
            new_picture = st.file_uploader("Bild hochladen", type=["png", "jpg", "jpeg"])
            submit_button = st.form_submit_button("Person hinzufügen")

            if submit_button:
                if new_id and new_date_of_birth and new_firstname and new_lastname and new_picture:
                    # Prüfen, ob ID eindeutig ist
                    if any(p.id == new_id for p in persons):
                        st.error("ID existiert bereits. Bitte wähle eine andere ID.")
                    else:
                        # Bild speichern
                        picture_path = os.path.join(IMAGE_DIR, f"{new_lastname}_{new_firstname}_{new_id}.{new_picture.name.split('.')[-1]}")
                        with open(picture_path, "wb") as f:
                            f.write(new_picture.getbuffer())
                        # Neue Person hinzufügen
                        Person.add_person(new_id, new_date_of_birth, new_firstname, new_lastname, picture_path, new_gender)
                        st.success(f"Person {new_lastname}, {new_firstname} erfolgreich hinzugefügt!")
                        # Seite neu laden, um die Liste zu aktualisieren
                        st.rerun()
                else:
                    st.error("Bitte fülle alle Felder aus und lade ein Bild hoch.")

    with tab3:
        st.header("Person bearbeiten")
        if person_names:
            selected_name = st.selectbox("Wähle eine Person zum Bearbeiten", options=person_names, key="edit_person")
            selected_person = Person.find_person_data_by_name(selected_name)
            if selected_person:
                with st.form("edit_person_form"):
                    edit_id = st.number_input("ID", min_value=1, value=selected_person.id, step=1)
                    edit_date_of_birth = st.number_input("Geburtsjahr", min_value=1900, max_value=date.today().year, value=int(selected_person.date_of_birth), step=1)
                    edit_firstname = st.text_input("Vorname", value=selected_person.firstname)
                    edit_lastname = st.text_input("Nachname", value=selected_person.lastname)
                    # Sicherstellen, dass ungültige Geschlechtswerte behandelt werden
                    gender_options = ["Male", "Female", "Other"]
                    current_gender = Person._normalize_gender(selected_person.gender)
                    edit_gender = st.selectbox("Geschlecht", options=gender_options, index=gender_options.index(current_gender))
                    edit_picture = st.file_uploader("Neues Bild hochladen (optional)", type=["png", "jpg", "jpeg"])
                    submit_button = st.form_submit_button("Änderungen speichern")

                    if submit_button:
                        if edit_id and edit_date_of_birth and edit_firstname and edit_lastname:
                            # Prüfen, ob ID eindeutig ist (außer für die aktuelle Person)
                            if any(p.id == edit_id and p.get_full_name() != selected_name for p in persons):
                                st.error("ID existiert bereits. Bitte wähle eine andere ID.")
                            else:
                                updated_data = {
                                    "id": edit_id,
                                    "date_of_birth": edit_date_of_birth,
                                    "firstname": edit_firstname,
                                    "lastname": edit_lastname,
                                    "gender": edit_gender,
                                    "ekg_tests": selected_person.ekg_tests,
                                    "is_new": selected_person.is_new
                                }
                                if edit_picture:
                                    # Altes Bild löschen, wenn es existiert
                                    if os.path.exists(selected_person.picture_path):
                                        os.remove(selected_person.picture_path)
                                    # Neues Bild speichern
                                    picture_path = os.path.join(IMAGE_DIR, f"{edit_lastname}_{edit_firstname}_{edit_id}.{edit_picture.name.split('.')[-1]}")
                                    with open(picture_path, "wb") as f:
                                        f.write(edit_picture.getbuffer())
                                    updated_data["picture_path"] = picture_path
                                else:
                                    # Alten Bildpfad beibehalten, aber an neuen Namen anpassen
                                    old_ext = selected_person.picture_path.split('.')[-1]
                                    new_picture_path = os.path.join(IMAGE_DIR, f"{edit_lastname}_{edit_firstname}_{edit_id}.{old_ext}")
                                    if os.path.exists(selected_person.picture_path):
                                        os.replace(selected_person.picture_path, new_picture_path)
                                    updated_data["picture_path"] = new_picture_path
                                Person.update_person(selected_name, updated_data)
                                st.success(f"Person {edit_lastname}, {edit_firstname} erfolgreich aktualisiert!")
                                st.rerun()
                        else:
                            st.error("Bitte fülle alle Pflichtfelder aus.")
        else:
            st.write("Keine Personen verfügbar.")

    with tab4:
        st.header("Person löschen")
        new_persons = [p for p in persons if p.is_new]
        new_person_names = Person.get_person_list(new_persons)
        if new_person_names:
            selected_name = st.selectbox("Wähle eine neu erstellte Person zum Löschen", options=new_person_names, key="delete_person")
            if st.button("Person löschen"):
                selected_person = Person.find_person_data_by_name(selected_name)
                if selected_person and selected_person.is_new:
                    # Bild löschen, wenn es existiert
                    if os.path.exists(selected_person.picture_path):
                        os.remove(selected_person.picture_path)
                    Person.delete_person(selected_name)
                    st.success(f"Person {selected_name} erfolgreich gelöscht!")
                    st.rerun()
                else:
                    st.error("Diese Person kann nicht gelöscht werden.")
        else:
            st.write("Keine neu erstellten Personen verfügbar.")

    with tab5:
        st.header("EKG-Test hinzufügen")
        if person_names:
            selected_name = st.selectbox("Wähle eine Person", options=person_names, key="add_test_person")
            with st.form("add_test_form"):
                test_date = st.date_input("Testdatum", value=date.today())
                test_file = st.file_uploader("EKG-Datei hochladen", type=["csv", "txt"])
                submit_button = st.form_submit_button("EKG-Test hinzufügen")

                if submit_button and test_date and test_file:
                    # EKG-Datei speichern
                    test_path = os.path.join("data/ekg_tests", f"{selected_name.replace(', ', '_')}_{test_date}.{test_file.name.split('.')[-1]}")
                    os.makedirs("data/ekg_tests", exist_ok=True)
                    with open(test_path, "wb") as f:
                        f.write(test_file.getbuffer())
                    Person.add_ekg_test(selected_name, str(test_date), test_path)
                    st.success(f"EKG-Test für {selected_name} erfolgreich hinzugefügt!")
                    st.rerun()
                elif submit_button:
                    st.error("Bitte wähle ein Datum und lade eine Datei hoch.")
        else:
            st.write("Keine Personen verfügbar.")

# --- EKG-Anomalie-Analyse ---
elif page == "EKG-Anomalie-Analyse":
    st.header("EKG-Verlauf und Anomalie-Auswertung: MIT-BIH Datensatz 208")
    
    try:
        signal_df, annotation_df, anomaly_summary = analyze_ecg_anomalies('208', data_dir='mitdb/1.0.0')
    except Exception as e:
        st.error(f"Fehler beim Laden der Daten: {e}")
        st.info("Stelle sicher, dass du eine Internetverbindung hast oder die Dateien 208.dat, 208.hea und 208.atr lokal in C:\\Abschlussprojekt_2025 verfügbar sind.")
        signal_df, annotation_df, anomaly_summary = None, None, None
    
    if signal_df is not None and annotation_df is not None and anomaly_summary is not None:
        st.sidebar.header("Zeitfenster auswählen")
        max_time = signal_df['Time (s)'].max()
        time_range = st.sidebar.slider(
            "Zeitbereich (Sekunden)",
            min_value=0.0,
            max_value=float(max_time),
            value=(0.0, min(30.0, max_time)),
            step=0.1
        )
        
        filtered_signal_df = signal_df[
            (signal_df['Time (s)'] >= time_range[0]) & 
            (signal_df['Time (s)'] <= time_range[1])
        ]
        filtered_annotation_df = annotation_df[
            (annotation_df['Time (s)'] >= time_range[0]) & 
            (annotation_df['Time (s)'] <= time_range[1]) & 
            (annotation_df['Symbol'] != 'N')
        ]
        
        total_beats = len(annotation_df)
        anomaly_beats = len(annotation_df[annotation_df['Symbol'] != 'N'])
        anomaly_rate = (anomaly_beats / total_beats * 100) if total_beats > 0 else 0
        duration_minutes = max_time / 60
        anomaly_freq_per_min = anomaly_beats / duration_minutes if duration_minutes > 0 else 0
        
        st.subheader("Allgemeine Statistiken")
        st.write(f"**Gesamtdauer**: {max_time:.2f} Sekunden ({duration_minutes:.2f} Minuten)")
        st.write(f"**Gesamtzahl der Schläge**: {total_beats}")
        st.write(f"**Anzahl der Anomalien**: {anomaly_beats} ({anomaly_rate:.2f}% der Schläge)")
        st.write(f"**Anomalien pro Minute**: {anomaly_freq_per_min:.2f}")
        
        st.subheader("EKG-Signale mit Anomalien")
        
        fig1 = go.Figure()
        fig1.add_trace(go.Scatter(
            x=filtered_signal_df['Time (s)'],
            y=filtered_signal_df['MLII (mV)'],
            mode='lines',
            name='MLII Signal',
            line=dict(color='blue')
        ))
        if not filtered_annotation_df.empty:
            anomaly_times = filtered_annotation_df['Time (s)']
            anomaly_values = filtered_signal_df.set_index('Time (s)')['MLII (mV)'].reindex(anomaly_times, method='nearest').values
            fig1.add_trace(go.Scatter(
                x=anomaly_times,
                y=anomaly_values,
                mode='markers',
                name='Anomalien',
                marker=dict(symbol='x', size=10, color='red'),
                text=filtered_annotation_df['Description'] + " (" + filtered_annotation_df['Time (s)'].round(2).astype(str) + "s)",
                hoverinfo='text'
            ))
        fig1.update_layout(
            title='EKG-Signal (MLII)',
            xaxis_title='Zeit (s)',
            yaxis_title='Amplitude (mV)',
            hovermode='x',
            showlegend=True
        )
        st.plotly_chart(fig1, use_container_width=True)
        
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(
            x=filtered_signal_df['Time (s)'],
            y=filtered_signal_df['V1 (mV)'],
            mode='lines',
            name='V1 Signal',
            line=dict(color='green')
        ))
        if not filtered_annotation_df.empty:
            anomaly_values_v1 = filtered_signal_df.set_index('Time (s)')['V1 (mV)'].reindex(anomaly_times, method='nearest').values
            fig2.add_trace(go.Scatter(
                x=anomaly_times,
                y=anomaly_values_v1,
                mode='markers',
                name='Anomalien',
                marker=dict(symbol='x', size=10, color='red'),
                text=filtered_annotation_df['Description'] + " (" + filtered_annotation_df['Time (s)'].round(2).astype(str) + "s)",
                hoverinfo='text'
            ))
        fig2.update_layout(
            title='EKG-Signal (V1)',
            xaxis_title='Zeit (s)',
            yaxis_title='Amplitude (mV)',
            hovermode='x',
            showlegend=True
        )
        st.plotly_chart(fig2, use_container_width=True)
        
        st.subheader("Zusammenfassung der Anomalien")
        clinical_context = {
            'V': "Ventrikuläre Extrasystole (VEB): Vorzeitiger Schlag aus den Ventrikeln. Häufige VEBs können auf Herzkrankheiten hinweisen.",
            'S': "Supraventrikuläre Extrasystole (SVEB): Vorzeitiger Schlag aus den Vorhöfen. Oft weniger schwerwiegend, aber kann Vorhofflimmern anzeigen.",
            'F': "Fusionsschlag: Mischung aus normalem und ektopischem Schlag. Weist auf komplexe Arrhythmien hin.",
            'Q': "Nicht klassifizierbarer Schlag: Unklarer Ursprung, selten.",
            '|': "Ausgefallener Schlag: Zeigt Pausen oder blockierte Impulse.",
            '/': "Schrittmacherschlag: Durch einen Herzschrittmacher ausgelöst."
        }
        
        anomaly_table_data = []
        unknown_count = 0
        unknown_times = []
        for symbol, info in anomaly_summary.items():
            times_in_range = [t for t in info['times'] if time_range[0] <= t <= time_range[1]]
            if symbol not in [ 'V', 'S', 'F', 'Q', '|', '/' ]:
                unknown_count += info['count']
                unknown_times.extend(times_in_range)
            else:
                times_str = ", ".join([f"{t:.2f}s" for t in times_in_range[:5]])
                if len(times_in_range) > 5:
                    times_str += "..."
                anomaly_table_data.append({
                    "Symbol": symbol,
                    "Beschreibung": info['description'],
                    "Anzahl": info['count'],
                    "Klinische Bedeutung": clinical_context.get(symbol, "Keine spezifische Information verfügbar."),
                    "Zeiten im Bereich": times_str if times_in_range else "Keine"
                })
        
        if unknown_count > 0:
            times_str = ", ".join([f"{t:.2f}s" for t in unknown_times[:5]])
            if len(unknown_times) > 5:
                times_str += "..."
            anomaly_table_data.append({
                "Symbol": "Unbekannt",
                "Beschreibung": "Unbekannte Anomalie",
                "Anzahl": unknown_count,
                "Klinische Bedeutung": "Keine spezifische Information verfügbar.",
                "Zeiten im Bereich": times_str if unknown_times else "Keine"
            })
        
        anomaly_table_df = pd.DataFrame(anomaly_table_data)
        st.dataframe(
            anomaly_table_df,
            use_container_width=True,
            column_config={
                "Symbol": st.column_config.TextColumn("Symbol", width="small"),
                "Beschreibung": st.column_config.TextColumn("Beschreibung", width="medium"),
                "Anzahl": st.column_config.NumberColumn("Anzahl", width="small"),
                "Klinische Bedeutung": st.column_config.TextColumn("Klinische Bedeutung", width="large"),
                "Zeiten im Bereich": st.column_config.TextColumn("Zeiten im Bereich", width="medium")
            }
        )
        
        st.subheader("Anomalien im ausgewählten Zeitbereich")
        if not filtered_annotation_df.empty:
            st.dataframe(
                filtered_annotation_df[['Time (s)', 'Symbol', 'Description']].round({'Time (s)': 2}),
                use_container_width=True,
                column_config={
                    "Time (s)": st.column_config.NumberColumn("Zeit (s)", format="%.2f"),
                    "Symbol": st.column_config.TextColumn("Symbol"),
                    "Description": st.column_config.TextColumn("Beschreibung")
                }
            )
        else:
            st.write("Keine Anomalien im ausgewählten Zeitbereich.")
"""

import streamlit as st
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
import pydeck as pdk
import os
import shutil
from PIL import Image
from datetime import date
from src.analyze_fit_files_tour import analyze_all_fit_files
from src.analyze_hoehenkurve import analyze_wildschoenau, analyze_pillersee
from src.analyze_data_GPS import analyze_wildschoenau as analyze_gps_wildschoenau, analyze_pillersee as analyze_gps_pillersee
from src.ekgdata import EKGdata
from src.person import Person
from src.analyze_activity_data import dataplot, dataframe
from src.analyze_anomalien import analyze_ecg_anomalies

# Streamlit-Seitenkonfiguration
st.set_page_config(page_title="Touren & EKG Analyse", layout="wide")

# Verzeichnis für Bild-Uploads
IMAGE_DIR = "data/images"
os.makedirs(IMAGE_DIR, exist_ok=True)

# Basisverzeichnis für FIT-Dateien (anpassbar)
BASE_DIR = "C:/Abschlussprojekt_2025/fit-files"
if not os.path.exists(BASE_DIR):
    st.warning(f"Das Verzeichnis {BASE_DIR} existiert nicht. Bitte überprüfe den Pfad.")
    BASE_DIR = "fit-files"  # Fallback-Verzeichnis
    os.makedirs(BASE_DIR, exist_ok=True)

# Sidebar für Navigation
page = st.sidebar.selectbox("Wähle eine Analyse", ["Touren-Analyse", "Personen- & EKG-Verwaltung", "EKG-Anomalie-Analyse"])

# --- Touren-Analyse (FIT, GPX, Höhenkurve) ---
if page == "Touren-Analyse":
    st.header("Touren-Analyse")

    # Tourauswahl
    tour = st.selectbox("Wähle eine Tour", ("Wildschönau", "Pillersee"), key="tour_select")

    # --- FIT-Dateien Analyse ---
    fit_files = [f for f in os.listdir(BASE_DIR) if f.endswith('.csv')]
    if not fit_files:
        st.error(f"Keine CSV-Dateien im Verzeichnis {BASE_DIR} gefunden.")
        st.stop()

    try:
        results = analyze_all_fit_files(BASE_DIR)
        if not results:
            st.error("Keine gültigen Dateien konnten verarbeitet werden.")
            st.stop()
    except Exception as e:
        st.error(f"Fehler beim Verarbeiten der FIT-Dateien: {e}")
        st.stop()

    # Flexibler Dateinamenabgleich
    tour_lower = tour.lower().replace('ö', 'oe')
    selected_result = None
    for r in results:
        file_name_lower = r['file_name'].lower()
        if tour_lower in file_name_lower or file_name_lower.startswith(tour_lower[:5]):
            selected_result = r
            break

    if not selected_result:
        st.error(f"Keine Daten für die ausgewählte Tour {tour} gefunden.")
        st.stop()

    # --- GPX-Analyse und Karte ---
    st.subheader("Karte")
    try:
        track_data = analyze_gps_wildschoenau() if tour == "Wildschönau" else analyze_gps_pillersee()
        if track_data.get("coordinates"):
            lats = [lat for lon, lat in track_data["coordinates"]]
            lons = [lon for lon, lat in track_data["coordinates"]]
            midpoint = [sum(lats)/len(lats), sum(lons)/len(lons)]
            layer = pdk.Layer(
                "PathLayer",
                data=[{"path": track_data["coordinates"], "name": tour}],
                get_path="path",
                get_color=[255, 0, 0],
                width_scale=10,
                width_min_pixels=3,
            )
            view_state = pdk.ViewState(
                latitude=midpoint[0],
                longitude=midpoint[1],
                zoom=13,
                pitch=0,
            )
            st.pydeck_chart(
                pdk.Deck(
                    layers=[layer],
                    initial_view_state=view_state,
                    map_style="mapbox://styles/mapbox/outdoors-v11"
                )
            )
        else:
            st.write("Keine Koordinaten gefunden.")
    except Exception as e:
        st.error(f"Fehler beim Laden der Kartendaten: {e}")
        st.stop()

    # --- Zusammenfassung und Diagramme ---
    st.subheader("Zusammenfassung")
    table_data = {
        "Tour": [tour],
        "Höhenmeter (m)": [selected_result.get('elevation_gain_m', 0)],
        "Gesamtdistanz (km)": [selected_result.get('total_distance_km', 0)],
        "Durchschnittsherzfrequenz (bpm)": [round(sum(selected_result.get('heart_rate', [])) / len(selected_result['heart_rate']), 2) if selected_result.get('heart_rate') else 0],
        "Durchschnittsleistung (Watt)": [round(sum(selected_result.get('power', [])) / len(selected_result['power']), 2) if selected_result.get('power') else 0]
    }
    df = pd.DataFrame(table_data)
    st.dataframe(
        df,
        use_container_width=True,
        column_config={
            "Tour": st.column_config.TextColumn("Tour", width="medium"),
            "Höhenmeter (m)": st.column_config.NumberColumn("Höhenmeter (m)", format="%.2f"),
            "Gesamtdistanz (km)": st.column_config.NumberColumn("Gesamtdistanz (km)", format="%.2f"),
            "Durchschnittsherzfrequenz (bpm)": st.column_config.NumberColumn("Ø Herzfrequenz (bpm)", format="%.2f"),
            "Durchschnittsleistung (Watt)": st.column_config.NumberColumn("Ø Leistung (Watt)", format="%.2f")
        }
    )

    try:
        st.subheader("Herzfrequenz und Leistung")
        df_csv = pd.read_csv(os.path.join(BASE_DIR, selected_result['file_name']))
        fig_combined = go.Figure()
        fig_combined.add_trace(
            go.Scatter(
                x=df_csv['Distance'],
                y=selected_result.get('heart_rate', []),
                mode='lines',
                name='Herzfrequenz',
                line=dict(color='blue'),
                yaxis='y1'
            )
        )
        fig_combined.add_trace(
            go.Scatter(
                x=df_csv['Distance'],
                y=selected_result.get('power', []),
                mode='lines',
                name='Leistung',
                line=dict(color='green'),
                yaxis='y2'
            )
        )
        fig_combined.update_layout(
            title="Herzfrequenz und Leistungsverlauf",
            xaxis=dict(title="Distanz (km)"),
            yaxis=dict(
                title=dict(text="Herzfrequenz (bpm)", font=dict(color="blue")),
                tickfont=dict(color="blue")
            ),
            yaxis2=dict(
                title=dict(text="Leistung (Watt)", font=dict(color="green")),
                tickfont=dict(color="green"),
                overlaying='y',
                side='right'
            ),
            hovermode='x unified',
            showlegend=True
        )
        st.plotly_chart(fig_combined, use_container_width=True)
    except Exception as e:
        st.error(f"Fehler beim Laden der Daten: {e}")

    # --- Höhenkurve ---
    st.subheader("Höhenprofil")
    try:
        df = analyze_wildschoenau() if tour == "Wildschönau" else analyze_pillersee()
    except Exception as e:
        st.error(f"Fehler beim Laden der Höhenkurve-Daten: {e}")
        st.stop()

    df = df.drop_duplicates(subset='Distance').sort_values('Distance')
    new_distance = pd.Series(range(int(df['Distance'].min() * 1000), 
                                  int(df['Distance'].max() * 1000) + 1)) / 1000
    df = pd.DataFrame({'Distance': new_distance}).merge(
        df[['Distance', 'AltitudeCorrected']], 
        on='Distance', 
        how='left'
    ).interpolate(method='linear')

    df['Slope'] = (df['AltitudeCorrected'].diff() / (df['Distance'].diff() * 1000)) * 100
    df['Slope'] = df['Slope'].fillna(0)

    fig = go.Figure(
        data=[
            go.Scatter(
                x=df['Distance'],
                y=df['AltitudeCorrected'],
                mode='lines',
                line=dict(color='blue', width=2),
                hovertemplate='Distanz: %{x:.2f} km<br>Höhe: %{y:.2f} m<br>Steigung: %{customdata:.2f}%',
                customdata=df['Slope']
            )
        ],
        layout=dict(
            title="Höhenprofil",
            xaxis_title="Distanz (km)",
            yaxis_title="Höhe (m)",
            showlegend=False,
            hovermode='x unified'
        )
    )
    st.plotly_chart(fig, use_container_width=True)

# --- Personen- & EKG-Verwaltung ---
elif page == "Personen- & EKG-Verwaltung":
    st.title("Personen- und EKG-Verwaltung")

    # Registerkarten für verschiedene Funktionen
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Person anzeigen", "Neue Person hinzufügen", "Person bearbeiten", "Person löschen", "EKG-Test hinzufügen"])

    # Personen laden
    try:
        persons = Person.get_person_data()
        person_names = Person.get_person_list(persons)
    except Exception as e:
        st.error(f"Fehler beim Laden der Personendaten: {e}")
        persons = []
        person_names = []

    with tab1:
        st.header("Person anzeigen")
        if person_names:
            selected_name = st.selectbox("Wähle eine Person", options=person_names, key="view_person")
            selected_person = Person.find_person_data_by_name(selected_name)
            if selected_person:
                try:
                    st.image(selected_person.get_image(), caption=selected_name)
                    st.write(f"Geburtsjahr: {selected_person.date_of_birth}")
                    st.write(f"Alter: {selected_person.calc_age()} Jahre")
                    st.write(f"Geschlecht: {selected_person.gender}")
                    st.write(f"Maximale Herzfrequenz: {selected_person.hr_max}")

                    # EKG-Tests anzeigen
                    if selected_person.ekg_tests:
                        selected_test = st.selectbox("Wähle einen EKG-Test", options=[str(i + 1) for i in range(len(selected_person.ekg_tests))], key="view_test")
                        testdatum = selected_person.ekg_tests[int(selected_test) - 1]["date"]
                        st.write(f"Datum des Tests: {testdatum}")

                        # Analyse-Plot anzeigen
                        hr_max = st.number_input("Maximale Herzfrequenz", min_value=100, max_value=250, value=int(selected_person.hr_max), step=1, key="view_hr_max")
                        try:
                            st.plotly_chart(dataplot(hr_max))
                        except Exception as e:
                            st.error(f"Fehler beim Erstellen des Analyse-Plots: {e}")

                        # Zonenstatistik
                        st.header("Zonenstatistik")
                        try:
                            zone_minutes = (dataframe["Zone"].value_counts() / 60)
                            zone_minutes.index.name = "Zone"
                            zone_minutes.name = "Dauer (Minuten)"
                            zone_power = dataframe.groupby("Zone")["PowerOriginal"].mean()
                            zone_stats = zone_minutes.to_frame().join(zone_power.rename("Ø Power (W)"))
                            st.dataframe(zone_stats)
                        except Exception as e:
                            st.error(f"Fehler bei der Zonenstatistik: {e}")

                        # EKG-Analyse
                        try:
                            ekg_obj = EKGdata(selected_person.ekg_tests[int(selected_test) - 1])
                            ekg_obj.find_peaks()
                            hr_est = ekg_obj.estimate_hr()
                            st.write(f"Geschätzte Herzfrequenz aus EKG: {hr_est:.1f} bpm")
                        except Exception as e:
                            st.error(f"Fehler bei der EKG-Analyse: {e}")
                    else:
                        st.write("Keine EKG-Daten für diese Person verfügbar.")
                except Exception as e:
                    st.error(f"Fehler beim Anzeigen der Personendaten: {e}")
            else:
                st.write("Person nicht gefunden.")
        else:
            st.write("Keine Personen verfügbar.")

    with tab2:
        st.header("Neue Person hinzufügen")
        with st.form("add_person_form"):
            new_id = st.number_input("ID", min_value=1, step=1)
            new_date_of_birth = st.number_input("Geburtsjahr", min_value=1900, max_value=date.today().year, step=1)
            new_firstname = st.text_input("Vorname")
            new_lastname = st.text_input("Nachname")
            new_gender = st.selectbox("Geschlecht", options=["Male", "Female", "Other"])
            new_picture = st.file_uploader("Bild hochladen", type=["png", "jpg", "jpeg"])
            submit_button = st.form_submit_button("Person hinzufügen")

            if submit_button:
                if new_id and new_date_of_birth and new_firstname and new_lastname and new_picture:
                    try:
                        # Prüfen, ob ID eindeutig ist
                        if any(p.id == new_id for p in persons):
                            st.error("ID existiert bereits. Bitte wähle eine andere ID.")
                        else:
                            # Bild speichern
                            picture_path = os.path.join(IMAGE_DIR, f"{new_lastname}_{new_firstname}_{new_id}.{new_picture.name.split('.')[-1]}")
                            with open(picture_path, "wb") as f:
                                f.write(new_picture.getbuffer())
                            # Neue Person hinzufügen
                            Person.add_person(new_id, new_date_of_birth, new_firstname, new_lastname, picture_path, new_gender)
                            st.success(f"Person {new_lastname}, {new_firstname} erfolgreich hinzugefügt!")
                            st.rerun()
                    except Exception as e:
                        st.error(f"Fehler beim Hinzufügen der Person: {e}")
                else:
                    st.error("Bitte fülle alle Felder aus und lade ein Bild hoch.")

    with tab3:
        st.header("Person bearbeiten")
        if person_names:
            selected_name = st.selectbox("Wähle eine Person zum Bearbeiten", options=person_names, key="edit_person")
            selected_person = Person.find_person_data_by_name(selected_name)
            if selected_person:
                with st.form("edit_person_form"):
                    edit_id = st.number_input("ID", min_value=1, value=selected_person.id, step=1)
                    edit_date_of_birth = st.number_input("Geburtsjahr", min_value=1900, max_value=date.today().year, value=int(selected_person.date_of_birth), step=1)
                    edit_firstname = st.text_input("Vorname", value=selected_person.firstname)
                    edit_lastname = st.text_input("Nachname", value=selected_person.lastname)
                    # Sicherstellen, dass ungültige Geschlechtswerte behandelt werden
                    gender_options = ["Male", "Female", "Other"]
                    try:
                        current_gender = Person._normalize_gender(selected_person.gender)
                    except AttributeError:
                        current_gender = selected_person.gender if selected_person.gender in gender_options else "Other"
                    edit_gender = st.selectbox("Geschlecht", options=gender_options, index=gender_options.index(current_gender))
                    edit_picture = st.file_uploader("Neues Bild hochladen (optional)", type=["png", "jpg", "jpeg"])
                    submit_button = st.form_submit_button("Änderungen speichern")

                    if submit_button:
                        if edit_id and edit_date_of_birth and edit_firstname and edit_lastname:
                            try:
                                # Prüfen, ob ID eindeutig ist (außer für die aktuelle Person)
                                if any(p.id == edit_id and p.get_full_name() != selected_name for p in persons):
                                    st.error("ID existiert bereits. Bitte wähle eine andere ID.")
                                else:
                                    updated_data = {
                                        "id": edit_id,
                                        "date_of_birth": edit_date_of_birth,
                                        "firstname": edit_firstname,
                                        "lastname": edit_lastname,
                                        "gender": edit_gender,
                                        "ekg_tests": selected_person.ekg_tests,
                                        "is_new": selected_person.is_new
                                    }
                                    if edit_picture:
                                        # Altes Bild löschen, wenn es existiert
                                        if os.path.exists(selected_person.picture_path):
                                            os.remove(selected_person.picture_path)
                                        # Neues Bild speichern
                                        picture_path = os.path.join(IMAGE_DIR, f"{edit_lastname}_{edit_firstname}_{edit_id}.{edit_picture.name.split('.')[-1]}")
                                        with open(picture_path, "wb") as f:
                                            f.write(edit_picture.getbuffer())
                                        updated_data["picture_path"] = picture_path
                                    else:
                                        # Alten Bildpfad beibehalten, aber an neuen Namen anpassen
                                        old_ext = selected_person.picture_path.split('.')[-1]
                                        new_picture_path = os.path.join(IMAGE_DIR, f"{edit_lastname}_{edit_firstname}_{edit_id}.{old_ext}")
                                        if os.path.exists(selected_person.picture_path):
                                            shutil.move(selected_person.picture_path, new_picture_path)
                                        updated_data["picture_path"] = new_picture_path
                                    Person.update_person(selected_name, updated_data)
                                    st.success(f"Person {edit_lastname}, {edit_firstname} erfolgreich aktualisiert!")
                                    st.rerun()
                            except Exception as e:
                                st.error(f"Fehler beim Aktualisieren der Person: {e}")
                        else:
                            st.error("Bitte fülle alle Pflichtfelder aus.")
        else:
            st.write("Keine Personen verfügbar.")

    with tab4:
        st.header("Person löschen")
        new_persons = [p for p in persons if p.is_new]
        new_person_names = Person.get_person_list(new_persons)
        if new_person_names:
            selected_name = st.selectbox("Wähle eine neu erstellte Person zum Löschen", options=new_person_names, key="delete_person")
            if st.button("Person löschen"):
                selected_person = Person.find_person_data_by_name(selected_name)
                if selected_person and selected_person.is_new:
                    try:
                        # Bild löschen, wenn es existiert
                        if os.path.exists(selected_person.picture_path):
                            os.remove(selected_person.picture_path)
                        Person.delete_person(selected_name)
                        st.success(f"Person {selected_name} erfolgreich gelöscht!")
                        st.rerun()
                    except Exception as e:
                        st.error(f"Fehler beim Löschen der Person: {e}")
                else:
                    st.error("Diese Person kann nicht gelöscht werden.")
        else:
            st.write("Keine neu erstellten Personen verfügbar.")

    with tab5:
        st.header("EKG-Test hinzufügen")
        if person_names:
            selected_name = st.selectbox("Wähle eine Person", options=person_names, key="add_test_person")
            with st.form("add_test_form"):
                test_date = st.date_input("Testdatum", value=date.today())
                test_file = st.file_uploader("EKG-Datei hochladen", type=["csv", "txt"])
                submit_button = st.form_submit_button("EKG-Test hinzufügen")

                if submit_button and test_date and test_file:
                    try:
                        # EKG-Datei speichern
                        test_path = os.path.join("data/ekg_tests", f"{selected_name.replace(', ', '_')}_{test_date}.{test_file.name.split('.')[-1]}")
                        os.makedirs("data/ekg_tests", exist_ok=True)
                        with open(test_path, "wb") as f:
                            f.write(test_file.getbuffer())
                        Person.add_ekg_test(selected_name, str(test_date), test_path)
                        st.success(f"EKG-Test für {selected_name} erfolgreich hinzugefügt!")
                        st.rerun()
                    except Exception as e:
                        st.error(f"Fehler beim Hinzufügen des EKG-Tests: {e}")
                elif submit_button:
                    st.error("Bitte wähle ein Datum und lade eine Datei hoch.")
        else:
            st.write("Keine Personen verfügbar.")

# --- EKG-Anomalie-Analyse ---
elif page == "EKG-Anomalie-Analyse":
    st.header("EKG-Verlauf und Anomalie-Auswertung: MIT-BIH Datensatz 208")
    
    try:
        signal_df, annotation_df, anomaly_summary = analyze_ecg_anomalies('208', data_dir='mitdb/1.0.0')
    except Exception as e:
        st.error(f"Fehler beim Laden der Daten: {e}")
        st.info("Stelle sicher, dass du eine Internetverbindung hast oder die Dateien 208.dat, 208.hea und 208.atr lokal in C:\\Abschlussprojekt_2025 verfügbar sind.")
        signal_df, annotation_df, anomaly_summary = None, None, None
    
    if signal_df is not None and annotation_df is not None and anomaly_summary is not None:
        st.sidebar.header("Zeitfenster auswählen")
        max_time = signal_df['Time (s)'].max()
        time_range = st.sidebar.slider(
            "Zeitbereich (Sekunden)",
            min_value=0.0,
            max_value=float(max_time),
            value=(0.0, min(30.0, max_time)),
            step=0.1
        )
        
        filtered_signal_df = signal_df[
            (signal_df['Time (s)'] >= time_range[0]) & 
            (signal_df['Time (s)'] <= time_range[1])
        ]
        filtered_annotation_df = annotation_df[
            (annotation_df['Time (s)'] >= time_range[0]) & 
            (annotation_df['Time (s)'] <= time_range[1]) & 
            (annotation_df['Symbol'] != 'N')
        ]
        
        total_beats = len(annotation_df)
        anomaly_beats = len(annotation_df[annotation_df['Symbol'] != 'N'])
        anomaly_rate = (anomaly_beats / total_beats * 100) if total_beats > 0 else 0
        duration_minutes = max_time / 60
        anomaly_freq_per_min = anomaly_beats / duration_minutes if duration_minutes > 0 else 0
        
        st.subheader("Allgemeine Statistiken")
        st.write(f"**Gesamtdauer**: {max_time:.2f} Sekunden ({duration_minutes:.2f} Minuten)")
        st.write(f"**Gesamtzahl der Schläge**: {total_beats}")
        st.write(f"**Anzahl der Anomalien**: {anomaly_beats} ({anomaly_rate:.2f}% der Schläge)")
        st.write(f"**Anomalien pro Minute**: {anomaly_freq_per_min:.2f}")
        
        st.subheader("EKG-Signale mit Anomalien")
        st.markdown("""
            <style>
            .small-text {
                font-size: 12px;
                color: #666666;
            }
            </style>
            <div class="small-text">
                <b>MLII</b>: Modifizierte Ableitung II, zeigt die Herzaktivität aus einer unteren Perspektive, ideal für die Erkennung von Arrhythmien des linken Ventrikels.<br>
                <b>V1</b>: Brustableitung, zeigt die Aktivität des rechten Ventrikels und des Septums, ergänzt MLII für eine umfassendere Analyse.
            </div>
        """, unsafe_allow_html=True)
        
        try:
            fig1 = go.Figure()
            fig1.add_trace(go.Scatter(
                x=filtered_signal_df['Time (s)'],
                y=filtered_signal_df['MLII (mV)'],
                mode='lines',
                name='MLII Signal',
                line=dict(color='blue')
            ))
            if not filtered_annotation_df.empty:
                anomaly_times = filtered_annotation_df['Time (s)']
                anomaly_values = filtered_signal_df.set_index('Time (s)')['MLII (mV)'].reindex(anomaly_times, method='nearest').values
                fig1.add_trace(go.Scatter(
                    x=anomaly_times,
                    y=anomaly_values,
                    mode='markers',
                    name='Anomalien',
                    marker=dict(symbol='x', size=10, color='red'),
                    text=filtered_annotation_df['Description'] + " (" + filtered_annotation_df['Time (s)'].round(2).astype(str) + "s)",
                    hoverinfo='text'
                ))
            fig1.update_layout(
                title='EKG-Signal (MLII)',
                xaxis_title='Zeit (s)',
                yaxis_title='Amplitude (mV)',
                hovermode='x',
                showlegend=True
            )
            st.plotly_chart(fig1, use_container_width=True)
            
            fig2 = go.Figure()
            fig2.add_trace(go.Scatter(
                x=filtered_signal_df['Time (s)'],
                y=filtered_signal_df['V1 (mV)'],
                mode='lines',
                name='V1 Signal',
                line=dict(color='green')
            ))
            if not filtered_annotation_df.empty:
                anomaly_values_v1 = filtered_signal_df.set_index('Time (s)')['V1 (mV)'].reindex(anomaly_times, method='nearest').values
                fig2.add_trace(go.Scatter(
                    x=anomaly_times,
                    y=anomaly_values_v1,
                    mode='markers',
                    name='Anomalien',
                    marker=dict(symbol='x', size=10, color='red'),
                    text=filtered_annotation_df['Description'] + " (" + filtered_annotation_df['Time (s)'].round(2).astype(str) + "s)",
                    hoverinfo='text'
                ))
            fig2.update_layout(
                title='EKG-Signal (V1)',
                xaxis_title='Zeit (s)',
                yaxis_title='Amplitude (mV)',
                hovermode='x',
                showlegend=True
            )
            st.plotly_chart(fig2, use_container_width=True)
        except Exception as e:
            st.error(f"Fehler beim Erstellen der EKG-Diagramme: {e}")

        st.subheader("Zusammenfassung der Anomalien")
        # CSS für Textumbruch in der Tabelle
        st.markdown("""
            <style>
            .stDataFrame table td:nth-child(4) {
                white-space: normal !important;
                word-wrap: break-word;
                max-width: 1000px;
            }
            </style>
        """, unsafe_allow_html=True)
        
        clinical_context = {
            'V': "Ventrikuläre Extrasystole (VEB): Vorzeitiger Schlag aus den Ventrikeln. Häufige VEBs können auf Herzkrankheiten hinweisen.",
            'S': "Supraventrikuläre Extrasystole (SVEB): Vorzeitiger Schlag aus den Vorhöfen. Oft weniger schwerwiegend, aber kann Vorhofflimmern anzeigen.",
            'F': "Fusionsschlag: Mischung aus normalem und ektopischem Schlag. Weist auf komplexe Arrhythmien hin.",
            'Q': "Nicht klassifizierbarer Schlag: Unklarer Ursprung, selten.",
            '|': "Ausgefallener Schlag: Zeigt Pausen oder blockierte Impulse.",
            '/': "Schrittmacherschlag: Durch einen Herzschrittmacher ausgelöst.",
            'U': "Unbekannte Anomalie: Nicht identifizierter Schlagtyp, möglicherweise unklassifizierbar."
        }
        
        anomaly_table_data = []
        unknown_count = 0
        try:
            for symbol, info in anomaly_summary.items():
                if not isinstance(symbol, str):
                    symbol = str(symbol)  # Sicherstellen, dass der Schlüssel eine Zeichenkette ist
                # Nur Anomalien mit einem Zeichen anzeigen
                if len(symbol) == 1 and symbol in ['V', 'S', 'F', 'Q', '|', '/']:
                    anomaly_table_data.append({
                        "Symbol": symbol,
                        "Beschreibung": info.get('description', 'Unbekannt'),
                        "Anzahl": info.get('count', 0),
                        "Klinische Bedeutung": clinical_context.get(symbol, "Keine spezifische Information verfügbar.")
                    })
                else:
                    unknown_count += info.get('count', 0)
        except Exception as e:
            st.error(f"Fehler beim Verarbeiten der Anomalien: {e}")

        if unknown_count > 0:
            anomaly_table_data.append({
                "Symbol": "U",
                "Beschreibung": "Unbekannte Anomalie",
                "Anzahl": unknown_count,
                "Klinische Bedeutung": clinical_context.get("U", "Keine spezifische Information verfügbar.")
            })
        
        anomaly_table_df = pd.DataFrame(anomaly_table_data)
        st.dataframe(
            anomaly_table_df,
            use_container_width=True,
            column_config={
                "Symbol": st.column_config.TextColumn("Symbol", width=100),
                "Beschreibung": st.column_config.TextColumn("Beschreibung", width=300),
                "Anzahl": st.column_config.NumberColumn("Anzahl", width=100),
                "Klinische Bedeutung": st.column_config.TextColumn("Klinische Bedeutung", width=1000)
            }
        )
        
        st.subheader("Anomalien im ausgewählten Zeitbereich")
        if not filtered_annotation_df.empty:
            # Nur Anomalien mit einem Zeichen oder unbekannte Anomalien (U) anzeigen
            filtered_annotation_df = filtered_annotation_df.copy()  # Kopie erstellen, um Original nicht zu verändern
            filtered_annotation_df['Symbol'] = filtered_annotation_df['Symbol'].apply(lambda x: x if len(x) == 1 and x in ['V', 'S', 'F', 'Q', '|', '/'] else 'U')
            st.dataframe(
                filtered_annotation_df[['Time (s)', 'Symbol', 'Description']].round({'Time (s)': 2}),
                use_container_width=True,
                column_config={
                    "Time (s)": st.column_config.NumberColumn("Zeit (s)", format="%.2f", width=150),
                    "Symbol": st.column_config.TextColumn("Symbol", width=100),
                    "Description": st.column_config.TextColumn("Beschreibung", width=400)
                }
            )
        else:
            st.write("Keine Anomalien im ausgewählten Zeitbereich.")